...
(async () => {
  ...
  console.log('Create:', cr.status, crText.slice(0,400));
  // Test con comillas simples en URL
  const url = S+\/SQLQueries('ldm_costos_bom')/List?code1='\'&code2='\'\;
  console.log('URL:', url);
  const tr = await fetch(url,{headers:{Cookie:c,Prefer:'odata.maxpagesize=200'}});
  console.log('Test:', tr.status, await tr.text().then((t:string)=>t.slice(0,600)));
