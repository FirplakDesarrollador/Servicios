...
(async () => {
  ...
  console.log('Test:', tr.status, 'Rows:', (td.value||[]).length);
  if(td.value&&td.value[0]) console.log('First row:', JSON.stringify(td.value[0]).slice(0,300));
  if(td.error) console.log('Error:', td.error.message.value);
